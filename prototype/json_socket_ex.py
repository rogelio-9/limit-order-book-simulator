...
async def handle_connection(websocket):
    ...
    try:
        async for message in websocket:
            try:
                ...
                print(f"Received trade")
                # Simulate order book response
                response = {
                    "status": "success",
                    "trade_id": trade.get("trade_id", "unknown"),
                    "received_time": time.ctime(),
                    "message": f"Trade {trade.get('side')} processed"
                }
    ...
    #     print(f"Standard deviation of time differences: {std:.6f} seconds")
